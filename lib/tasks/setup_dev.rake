...
task setup_dev: :environment do
  vagrant_vm_ip = if ENV["USER"] == "vagrant" # Anything else and we're running local, so just use 127.0.0.1
    ...
    if ENV["DEV_VM_IP"].blank?
      warn "DEV_VM_IP is not set; assuming the node runs on this machine (127.0.0.1)."
      warn "If your node is a separate VM, set DEV_VM_IP in .envrc and re-run."
    end
    ENV["DEV_VM_IP"].presence || "127.0.0.1"
  end
  ...
  ssh_client = DockerSSH::Node.new("ssh://#{vagrant_vm_ip}:22", {key: "#{Rails.root.join("#{ENV["CS_SSH_KEY"]}")}"})

  # Configure vagrant VM to communicate with controller
  #
  # Prefer an explicit CONTROLLER_IP; otherwise ask the kernel which local address
  # it would use to reach the VM. UDPSocket#connect on a datagram socket transmits
  # nothing -- it only asks the kernel which local source address it would route
  # from, so this is not a network call despite appearances.
  controller_ip = ENV["CONTROLLER_IP"].presence || if vagrant_vm_ip == "127.0.0.1"
    vagrant_vm_ip
  else
    require "socket"
    UDPSocket.open { |s| s.connect(vagrant_vm_ip, 1); s.addr.last }
  end
  ssh_client.client.exec!("echo '#{controller_ip} controller.cstacks.local' >> /etc/hosts")

  lb_cert = if ENV["USER"] == "vagrant"
    File.read("/home/cstacks/.ssl_wildcard/sharedcert.pem")
  elsif vagrant_vm_ip != "127.0.0.1"
    ssh_client.client.exec!("if [[ -f /home/cstacks/.ssl_wildcard/sharedcert.pem ]]; then cat /home/cstacks/.ssl_wildcard/sharedcert.pem; else echo ''; fi").strip
  else
    File.read("#{Rails.root.join("lib/dev/test_wildcard_ssl/sharedcert-test-crt")}")
  end
  ...
  region = if location.regions.empty?
    location.regions.create!(
      ...
      guac_url: "http://localhost:#{ENV["GUAC_PORT"]}/guacamole",
      guac_key: ENV["GUAC_KEY"]
    )
  else
  ...
  # Must match the tag lib/dev/single-node.sh preloads and the Setting default
  # in app/models/setting.rb; `:latest` here defeated the preload and pulled a
  # different image than production runs.
  Setting.find_by(name: "cs_bastion_image").update value: "ghcr.io/computestacks/cs-docker-bastion:v2"
  ...
  region.update metric_client: mc,
    ...
    acme_server: "#{controller_ip}:3005"
