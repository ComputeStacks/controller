...
if [ $(dpkg --print-architecture) == 'amd64' ]; then
  ...
  echo "deb [signed-by=/etc/apt/keyrings/auth-48-pub.asc arch=amd64] http://repo.powerdns.com/debian bullseye-auth-48 main" > /etc/apt/sources.list.d/pdns.list
...
apt-get update
apt-get -y install docker-ce=5:20.10.23~3-0~debian-bullseye docker-ce-cli=5:20.10.23~3-0~debian-bullseye containerd.io consul haproxy=2.8.\* pdns-server pdns-backend-pgsql
