...
block_collaborate.block_contents.create!(
  locale: ENV["LOCALE"],
  body: %(<div><strong>Warning!</strong> Any collaborator you add will have the same permissions as you!</div>)
)
block_faq = Block.create!(title: "Collaborate: FAQ", content_key: "collaborate.faq")
# noinspection RubyScope
block_faq.block_contents.create!(
  locale: ENV["LOCALE"],
  body: %(<div>Collaborators will be able to edit and manage this resource, as well as, create and delete any child resources. Any billable action they take will be billed to the resource owner's account.</div>)
)
...
block_collaborate.block_contents.create!(
  locale: ENV["LOCALE"],
  body: %(<h4>Container Registry Collaboration Invite</loadh4><div>{{ user }} has invited you to collaborate on the container registry {{ registry }}. Please click the link below to accept or reject this invitation.</div>)
)
...
block_collaborate.block_contents.create!(
  locale: ENV["LOCALE"],
  body: %(<h4>Project Collaboration Invite</h4><div>{{ user }} has invited you to collaborate on the project {{ project }}. Please click the link below to accept or reject this invitation.</div>)
)
...
block_collaborate.block_contents.create!(
  locale: ENV["LOCALE"],
  body: %(<h4>Container Image Collaboration Invite</h4><div>{{ user }} has invited you to collaborate on the image {{ image }}. Please click the link below to accept or reject this invitation.</div>)
)
...
block_collaborate.block_contents.create!(
  locale: ENV["LOCALE"],
  body: %(<h4>DNS Zone Collaboration Invite</h4><div>{{ user }} has invited you to collaborate on the zone {{ domain }}. Please click the link below to accept or reject this invitation.</div>)
)
...
# For non-prod, set acme directory to staging environment.
unless Rails.env.production?
  Setting.find_by(name: "acme_directory")&.update value: "https://acme-staging-v02.api.letsencrypt.org/directory"
end
