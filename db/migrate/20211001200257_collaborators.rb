class Collaborators < ActiveRecord::Migration[6.1]
  def change
    ...
    unless Block.where(content_key: "collaborate.invite.registry").exists?
      ...
      block_collaborate.block_contents.create!(
        locale: ENV["LOCALE"],
        body: %(<h4>Container Registry Collaboration Invite</h4><div>{{ user }} has invited you to collaborate on the container registry {{ registry }}. Please click the link below to accept or reject this invitation.</div>)
      )
    end
