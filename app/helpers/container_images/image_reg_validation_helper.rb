module ContainerImages
  module ImageRegValidationHelper
    ...
    def image_valid_tag_label(image, hide_valid = false)
      ...
      %(<span class="label label-#{status_class}"><i class="fa-solid #{status_icon}"></i></span>).html_safe
    end
    ...
    def image_valid_tag_message(image)
      ...
      when :partial
        %(<small class='text-danger'>Some image variants are not available. #{retry_link}</small>).html_safe
      when :pending
        "<small class='text-muted'>Pending registry validation...</small>".html_safe
      when :invalid
        %(<small class='text-danger'>Error connecting to registry, ensure settings are correct. #{retry_link}</small>).html_safe
      end
    end
