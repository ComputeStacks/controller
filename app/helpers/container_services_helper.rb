module ContainerServicesHelper
  ...
  def service_list_status(service)
    ...
    when "active_alert"
      %(<span style='color:rgb(217,175,95);'><i class="fa-solid fa-triangle-exclamation"></i> Active Alert</span>)
    when "unhealthy"
  ...
  def service_img_icon_tag(service)
    %(<img src="#{image_path service.container_image.icon_url}" style="cursor:pointer;max-width:35px;height:20px;padding-right:10px;" title="#{service.name}" onclick="window.location='#{container_service_path(service)}';" alt="#{service.name}" class="img-circle" />).html_safe
  end
  ...
  def service_list_status_indicator(service)
    ...
    when "inactive", "offline_containers"
      %( #{tag.i(nil, class: "text-warning fa-solid fa-triangle-exclamation")} Offline Containers ).html_safe
    when "alert", "active_alert", "resource_usage"
      %( #{tag.i(nil, class: "text-danger fa-solid fa-triangle-exclamation")} Alert ).html_safe
    when "working"
      "Operation In Progress"
    when "unhealthy"
      %( #{tag.i(nil, class: "text-danger fa-solid fa-triangle-exclamation")} Failing Health Checks ).html_safe
    else
