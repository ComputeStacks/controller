module ContainerRegistryHelper
  ...
  def container_registry_image_path(id, name)
    "#{/admin/.match?(request.path) ? "/admin" : ""}/container_images/#{id}-#{name.parameterize}"
  end

  def link_reg_to_image_path(registry, image, tag)
    "#{/admin/.match?(request.path) ? "/admin" : ""}/container_images/new?container_image[container_image_provider_id]=#{registry.container_image_provider.id}&container_image[registry_image_path]=#{image}&container_image[registry_image_tag]=#{tag}"
  end
end
