module NodeHelper
  # Takes an already-fetched boot time rather than a node: reading it from the
  # node here meant two Prometheus round trips per render, one for the nil check
  # and another for the value.
  def uptime_in_words(boot_time)
    boot_time.nil? ? "..." : distance_of_time_in_words_to_now(boot_time)
  rescue
    "..."
  end
