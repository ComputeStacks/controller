module BillingEventsHelper
  def billing_event_title(billing_event, perspective = "subscription")
    ...
    msg = if perspective == "subscription" || billing_event.subscription_product.nil?
      billing_event.url.nil? ? billing_event.subscription.label : "<a href='#{billing_event.url}'>#{billing_event.subscription.label}</a>"
    else
      billing_event.subscription_product.product.label
    end
    ...
    if source_product || destination_product
      msg = if perspective == "subscription" || billing_event.subscription_product.nil?
        "#{msg} product moved from #{source_product.nil? ? "None" : source_product.label} to #{destination_product.nil? ? "None" : destination_product.label}."
      else
        "#{msg} was migrated to #{destination_product.nil? ? "None" : destination_product.label}."
      end
    end
