    ...
    def ingress_domain_proto(domain)
      ...
      when "http"
        if domain.force_ssl?
          ...
          ingress_rule.backend_ssl ? "https" : %(<span title="Internet &rarr; #{lb_name} &rarr; #{domain.container_service.name}">https &rarr; http</span>).html_safe
        else
          ingress_rule.backend_ssl ? %(<span title="Internet &rarr; #{lb_name} &rarr; #{domain.container_service.name}">http &rarr; https</span>).html_safe : "http"
        end
      when "tcp"
        ingress_rule.backend_ssl ? %(<span title="Internet &rarr; #{lb_name} &rarr; #{domain.container_service.name}">tcp &rarr; tcp+tls</span>).html_safe : "tcp"
      when "tls"
        ingress_rule.backend_ssl ? "tcp+tls" : %(<span title="Internet &rarr; #{lb_name} &rarr; #{domain.container_service.name}">tcp+tls &rarr; tcp</span>).html_safe
      when "udp"
        "udp"
      else
        "..."
      ...
      service.ingress_rules.each do |ingress|
        ...
        when "mysql"
          if ingress.port == 3306
            cmd << %(mysql -u root -h #{temp_public_ip} -P #{ingress.port_nat} -p)
          end
        else
          next
        end
