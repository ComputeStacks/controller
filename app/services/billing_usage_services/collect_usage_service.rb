module BillingUsageServices
  ...
  class CollectUsageService
    ...
    def offline_storage(user)
      volumes = user.volumes.where.not(detached_at: nil)
      ...
      volumes.each do |vol|
        ...
        if backup_product
          ...
          backup_sb.billing_usages.order(created_at: :desc).first
        else
          backup_sb = nil
          nil
        end
        ...
        unless sb.billing_usages.where("period_start > ? and period_end <= ?", start_time, end_time).exists?
          ...
          usage_total = if product_rate&.price
            ((product_rate.price * period_length) * qty).round(8)
          else
            BigDecimal(0)
          end

          current_usage_rate = product_rate&.price
          current_usage_rate = BigDecimal(0) if current_usage_rate.nil?
