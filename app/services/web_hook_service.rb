class WebHookService
  ...
  def perform
    ...
    return false unless webhook_url&.match?(URI::DEFAULT_PARSER.make_regexp)
  ...
  def retry!(result_code = nil)
    ...
    if cached > 5
      ...
      if event
        event.event_details.create!(data: "Error, did not receive HTTP 2xx code from endpoint: #{result_code.nil? ? "ERR" : result_code}. Retry in #{wait_time}", event_code: "705055b10ccc0453")
      end
    end
  end
end
