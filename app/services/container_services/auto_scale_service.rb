  ...
  class AutoScaleService
    ...
    def new_package
      ...
      package = if resource_kind == "memory"
        BillingPackage.where(Arel.sql(%(cpu >= #{cur_cpu} AND memory > #{cur_mem}))).order(memory: :asc)[0]
      else
        BillingPackage.where(Arel.sql(%(cpu > #{cur_cpu} AND memory >= #{cur_mem}))).order(cpu: :asc)[0]
      end
