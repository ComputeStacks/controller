class SessionsController < Devise::SessionsController
  ...
  def create
    ...
    if resource
      if resource.require_2fa_auth?
        session[:req_second_factor] = true
        resource.update_attribute :req_second_factor, true
      else
        session.delete(:req_second_factor)
        resource.update_attribute :req_second_factor, false if resource.req_second_factor
      end
      resource.update_attribute :bypass_second_factor, false if resource.bypass_second_factor
      # set_flash_message :notice, :signed_in ## no longer want flash messages on login.
      sign_in(resource_name, resource)
      yield resource if block_given?
      flash.delete(:notice) if resource.req_second_factor
