class Admin::SearchController < Admin::ApplicationController
  ...
  def create
    ...
    unless params[:q].nil? || params[:q] == ""
      ...
      if /@/.match?(@q)
        sparams[:users] = User.where(Arel.sql(%(lower(email) LIKE '%#{@q}%')))
        ...
        sparams[:cidr] = Network::Cidr.where(cidr: @q) + Node.where(Arel.sql(%(primary_ip = '#{@q}' OR public_ip = '#{@q}' OR agent_host = '#{@q}')))
      elsif q_filter == "user"
        sparams[:users] = User.search_by @q
      elsif q_filter == "image"
        sparams[:images] = ContainerImage.where Arel.sql %(lower(name) ~ '#{@q}' OR lower(label) ~ '#{@q}')
      elsif q_filter == "deployment"
        sparams[:deployments] = Deployment.where(Arel.sql(%(lower(name) ~ '#{@q}' OR token LIKE '%#{@q}%')))
      elsif q_filter == "product"
        sparams[:product] = Product.where Arel.sql %(lower(name) ~ '#{@q}' OR lower(label) ~ '#{@q}')
      elsif q_filter == "domain"
        sparams[:container_domains] = Deployment::ContainerDomain.where(Arel.sql(%(domain ~ '#{@q}')))
      else
        sparams[:users] = User.search_by @q
        sparams[:deployments] = Deployment.where(Arel.sql(%(lower(name) ~ '#{@q}')))
        sparams[:containers] = Deployment::Container.where(Arel.sql(%(lower(name) ~ '#{@q}')))
        sparams[:volumes] = Volume.where(Arel.sql(%( lower(name) ~ '#{@q}' OR lower(label) ~ '#{@q}' )))
        sparams[:container_domains] = Deployment::ContainerDomain.where(Arel.sql(%(domain ~ '#{@q}')))
        sparams[:dns_zones] = ::Dns::Zone.where(Arel.sql(%( name ~ '#{@q}' )))
        sparams[:images] = ContainerImage.where Arel.sql %(lower(name) ~ '#{@q}' OR lower(label) ~ '#{@q}')
        sparams[:subscriptions] = Subscription.where Arel.sql %(lower(label) ~ '#{@q}')
        sparams[:product] = Product.where Arel.sql %(lower(name) ~ '#{@q}' OR lower(label) ~ '#{@q}')
        sparams[:sftp] = Deployment::Sftp.where(Arel.sql(%(lower(name) ~ '#{@q}')))
      end
    end
    @empty_results = true
    ...
    sparams[:images].each do |i|
      t = if i.user
        %(#{i.label} (<i class="fa-solid fa-user"></i> #{i.user.full_name}))
      else
        i.label
      end
      @search_params << {
    ...
    sparams[:subscriptions].each do |i|
      ...
      @search_params << {
        label: '<span class="label label-warning">Subscription</span>',
