class Admin::DeploymentsController < Admin::ApplicationController
  ...
  def index
    ...
    if params[:disk_usage] && params[:disk_usage].to_i > 0
      ...
      deployments = deployments.where(Arel.sql(%( id IN (#{disk_size_ids}) )))
    end
    unless params[:created_between].blank?
      begin
        ...
        deployments = deployments.where(Arel.sql(%( created_at BETWEEN '#{stime}' AND '#{etime}' )))
      rescue
    ...
    if params[:package].is_a?(Array)
      ...
      deployments = deployments.joins(subscriptions: {subscription_products: :product}).where(Arel.sql(%(subscription_products.product_id IN (#{package_ids})))).distinct
    end
