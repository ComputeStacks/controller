...
class Admin::VolumesController < Admin::ApplicationController
  ...
  def index
    ...
    volumes = case params[:projects]
    when "linked"
      volumes.where.not(deployment: nil)
    when "unlinked"
      volumes.where(deployment: nil)
    else
      volumes
    end
    ...
    @volumes = volumes.sorted.paginate per_page: 35, page: params[:page]
  end
  ...
  def destroy
    ...
    volume_ids += params[:volume_ids] if params[:volume_ids].is_a?(Array)
    volume_ids.flatten!
    if volume_ids.empty?
