class Deployments::SftpController < Deployments::BaseController
  ...
  def create
    ...
    ProjectWorkers::SftpInitWorker.perform_async @deployment.global_id, event.global_id
  ...
  def find_sftp
    ...
    redirect_to("/deployments/#{@deployment.token}", alert: "Unknown container") if @sftp.nil?
  end
end
