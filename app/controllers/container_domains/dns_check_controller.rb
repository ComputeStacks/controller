class ContainerDomains::DnsCheckController < AuthController
  ...
  def create
    ...
    elsif LetsEncryptWorkers::ValidateDomainWorker.new.perform(@domain.id)
  ...
  def find_domain
    ...
    redirect_to("/container_domains/#{@domain.id}/edit", alert: "Error, domain does not belong to a service.") if @deployment.nil?
  end
end
