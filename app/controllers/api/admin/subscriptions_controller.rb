...
class Api::Admin::SubscriptionsController < Api::Admin::ApplicationController
  ...
  def index
    if params[:user_id] && !params[:user_id].to_i.zero?
      ...
      @subscriptions = case params[:filter]
      when "active"
        user.subscriptions.where(active: true).order(created_at: :desc)
      when "inactive"
        user.subscriptions.where(active: false).order(created_at: :desc)
      else
        user.subscriptions.all.order(created_at: :desc)
      end
    else
      @subscriptions = case params[:filter]
      when "active"
        paginate Subscription.where(active: true).order(created_at: :desc)
      when "inactive"
        paginate Subscription.where(active: false).order(created_at: :desc)
      else
        paginate Subscription.all.order(created_at: :desc)
      end
    end
    respond_to :json, :xml
  end
  ...
  def update
    if params[:container_service] # upgrade / downgrade service
      ...
      # New Package
      unless pid.blank?
        product = if params[:container_service][:external_product]
          Product.find_by(external_id: pid)
        else
          Product.find_by(id: pid)
        end
  ...
  def load_subscription
    @subscription = if params[:find_by_external_id]
      Subscription.find_by(external_id: params[:id])
    else
