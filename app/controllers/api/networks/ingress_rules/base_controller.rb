class Api::Networks::IngressRules::BaseController < Api::ApplicationController
  ...
  def find_ingress_rule
    ...
    if @ingress_rule.container_service
      api_obj_no_access unless @ingress_rule.container_service.can_edit?(current_user)
    elsif @ingress_rule.sftp_container
      api_obj_no_access unless @ingress_rule.sftp_container.can_edit?(current_user)
    else
      api_obj_missing
    end
  end
