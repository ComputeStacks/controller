...
class Api::Stacks::LoadBalancersController < Api::Stacks::BaseController
  ...
  # GET /stacks/load_balancers/acl/:kind
  def acl
    ips = case params[:kind]
          when 'cloudflare'
            @load_balancer.cloudflare_ipaddrs
          when 'bunny'
            @load_balancer.bunny_ipaddrs
          else # proxy
            @load_balancer.proxy_ipaddrs
          end

    render plain: ips.map {|i| "#{i.to_s}/#{i.prefix}"}.join("\n")
  end
