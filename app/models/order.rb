...
class Order < ApplicationRecord
  ...
  def self.clean!
    Order.where(Arel.sql(%(status = 'open' and created_at < '#{1.week.ago.iso8601}'))).delete_all
    Order.where(Arel.sql(%(status = 'awaiting_payment' and created_at < '#{1.week.ago.iso8601}'))).update_all status: "cancelled"
  end
  ...
  def generate_details
    ...
    new_order["summary"] = []
    if raw_order.nil? || !raw_order.is_a?(Array)
    ...
    unless new_order["deployment"].nil?
      ...
      if new_order["deployment"]["id"] && deployment.nil?
