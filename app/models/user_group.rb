...
class UserGroup < ApplicationRecord
  ...
  def ensure_active_default
    update_column active, true if is_default && !active
  end

  # :doc:
  # Ensure we choose another default if this +is_default+ :doc:
  def update_default
    UserGroup.where("id != ?", id).update_all(is_default: false) if is_default
  end
  ...
  def ensure_new_default
    return true unless is_default
    if UserGroup.active.count < 2
      throw(:abort)
    else
      unless UserGroup.active.where.not(id: id).first.update_attribute(:is_default, true)
