...
class ProvisionDriver < ApplicationRecord
  ...
  def virtual_machine
    eval("#{module_name}::VirtualMachine")
  end

  def template
    eval("#{module_name}::Template")
  end

  def snapshot
    eval("#{module_name}::Snapshot")
  end
  ...
  def features
    eval("#{module_name}::Settings").available_actions
  end

  def host_settings
    eval("#{module_name}::Settings").host_settings(service_client)
  end

  def cloud_user
    eval("#{module_name}::User")
  end

  def module_settings
    eval("#{module_name}::Settings")
  end

  # TODO: Figure out what this is for...we're not actually using the settings.
  def sync_settings!
    response = module_settings.host_settings(service_client)
    if response
      ...
      update_attribute :settings, existing_settings
    end
  end

  # Will attempt to load all regions
  def load_regions!
    all_regions = module_settings.available_regions(service_client)
    ...
    if regions.where(is_default: true).empty?
      regions.first.update_attribute :is_default, true
    end
    "Driver now has #{regions.count} Regions."
  end
  ...
  def service_client
    eval("#{module_name}::Client").new(endpoint, cloud_auth)
  end

  def zone
    load_pd_config
    eval("#{module_name}::Dns::Zone")
  end

  def zone_record
    load_pd_config
    eval("#{module_name}::Dns::ZoneRecord")
  end
  ...
  def load_pd_config
    eval("#{module_name}").configure(settings["config"]) if settings["config"] && !settings["config"].empty?
  end

  def cloud_auth
    eval("#{module_name}::Auth").new(0, username, api_key.nil? ? nil : Secret.decrypt!(api_key), api_secret.nil? ? nil : Secret.decrypt!(api_secret))
  end
end
