...
class LetsEncrypt < ApplicationRecord
  ...
  def can_renew?
    ...
    if Rails.env.test? # The acme test server returns certificates with really short expirations.
      (expires_at < 3.days.from_now) || (expected_domains != names)
    else
  ...
  def has_default_acme_provider?
    account.acme_directory == LetsEncryptAccount.acme_directory
  end
  ...
  def can_provision?(event)
    ...
    if event_details.where("event_log_data.event_code = ?", "ef1da605ec1b3927").exists?
      event.event_details.create!(
        data: "Fatal ACME error, contact support to clear these events before proceeding.",
    ...
    if EventLogDatum.where("event_log_data.event_code = ? and event_log_data.created_at > ?", "befbf62f360b8958", 1.day.ago).exists?
      ...
      # full stop!
      event.event_details.create!(
        data: "Unable to generate certificate due to rate limit with ACME. Will attempt in 24 hours.",
        event_code: "befbf62f360b8958"
      )
      event.cancel! "Rate limited by ACME"
      return false
    end
    true
  end
