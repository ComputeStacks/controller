    ...
    class_methods do
      ...
      def clean_container_services!
        left_outer_joins(:container_services).where(
          ...
          Arel.sql(
            "event_logs.locale IN ('service.building', 'service.building_1', 'service.removing', 'service.removing_1', 'service.resizing', 'service.resizing_1', 'service.scaling', 'service.errors.scale', 'service.errors.resize', 'service.errors.delete')"
          )
        ).delete_all
      end
      ...
      def clean_containers!
        left_outer_joins(:containers).where(
          ...
          Arel.sql(
            "event_logs.locale IN ('container.start', 'container.stop', 'container.restart', 'container.delete', 'container.build', 'container.rebuild', 'container.errors.general_provision', 'container.errors.resize')"
          )
        ).delete_all
      end
      ...
      def clean_sftp!
        left_outer_joins(:sftp_containers).where(
          ...
          Arel.sql("event_logs.locale IN ('container.start', 'container.stop', 'container.restart', 'container.delete', 'container.build')")
        ).delete_all
      end
