module RemoteZone
  ...
  def module_delete!
    client.destroy if provision_driver
  rescue => e
  ...
  def available_actions
    provision_driver.module_settings.available_actions["dns"]
  rescue => e
  ...
  def apply_commitments
    return nil if provision_driver.nil?
    if ActiveRecord::Type::Boolean.new.cast(commit_changes)
      state_commit!
    elsif ActiveRecord::Type::Boolean.new.cast(revert_changes)
      state_revert!
    end
  end

  def module_create!
    if run_module_create.nil?
      # nil = do nothing.
    elsif provision_driver.nil?
      # Also do nothing..no provision driver.
    elsif ActiveRecord::Type::Boolean.new.cast(run_module_create)
      dns_client = client
      module_client = dns_client
      module_client.name = name
  ...
  def set_provision_driver
    return nil if ActiveRecord::Type::Boolean.new.cast(skip_provision_driver)
    if provision_driver_id.nil?
  ...
  def state_commit!
    if saved_state.nil? || !available_actions.include?("update_by_zone")
      {"success" => false, "message" => "Action not available."}
    else
      begin
        zone = state_load!
        zone.save
        ...
        update_columns(saved_state: nil, saved_state_ts: nil)
        {"success" => true}
      end
    end
  end
