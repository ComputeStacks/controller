module Volumes
  module BorgPolicy
    ...
    def validate_volume_consul
      ...
      self.borg_freq = borg_freq.strip
      custom_freq = %w[@hourly @midnight @weekly @monthly @daily]
      unless custom_freq.include?(borg_freq)
        parsed = Fugit::Cron.parse(borg_freq)
        if parsed.nil?
          errors.add(:cron_freq, "invalid frequency syntax")
        elsif parsed.rough_frequency < 3600 && borg_strategy != "file"
          errors.add(:cron_freq, "frequency must not exceed every hour")
        elsif parsed.rough_frequency < 1800 && borg_strategy == "file"
