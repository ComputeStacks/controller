...
class BillingResourcePrice < ApplicationRecord
  ...
  def self.available_currencies
    c = BillingResourcePrice.select(Arel.sql(%( DISTINCT(currency) ))).map { |i| i.currency }
  ...
  def set_defaults
    if billing_phase.nil?
      final_phase = billing_resource.billing_phases.find_by(phase_type: "final")
      if final_phase.nil?
        final_phase = billing_resource.billing_phases.create!(phase_type: "final")
      end
  ...
  def ensure_unique_max_qty
    ...
    if max_qty.nil?
      ...
      query = if id.blank?
        %( max_qty IS NULL AND regions.id IN (#{rids}) AND currency = '#{currency}' )
      else
        %( billing_resource_prices.id != #{id} AND (max_qty IS NULL AND regions.id IN (#{rids})) AND currency = '#{currency}' )
      end
      ...
      rids = regions.map { |i| i.id }.join(", ")
      query = if id.blank?
        %( max_qty = #{max_qty.to_f} AND regions.id IN (#{rids}) AND currency = '#{currency}' )
      else
        %( billing_resource_prices.id != #{id} AND (max_qty = #{max_qty.to_f} AND regions.id IN (#{rids})) AND currency = '#{currency}' )
      end
      if billing_phase
        if billing_phase.prices.where(Arel.sql(query)).joins(:regions).exists?
          errors.add(:max_qty, "value already exists")
