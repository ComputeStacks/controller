  ...
  def perform(alert_id)
    ...
    if alert.active? && alert.has_notifications?
      ...
      if deployment
        ...
        deployment.project_notifiers.where(Arel.sql(%('#{alert.name}' = ANY (rules)))).each do |i|
          i.current_event = event
          i.fire_alert! alert
        end

        deployment.user.user_notifications.where(Arel.sql(%('#{alert.name}' = ANY (rules)))).each do |i|
          i.current_event = event
          i.fire_alert! alert
        end
      ...
      SystemNotification.where(Arel.sql(%('#{alert.name}' = ANY (rules)))).each do |i|
        i.current_event = event
        i.fire_alert! alert
      end
    ...
    if alert.name == "NodeUp" && alert.node
      NodeWorkers::HeartbeatWorker.perform_async alert.node.global_id
    end

    # Container has gone away
    if alert.name == "ContainerKilled" && alert.active?
      if alert.container
        ContainerWorkers::RecoverContainerWorker.perform_async alert.container.global_id, alert.global_id
      end
      if alert.sftp_container
        ContainerWorkers::RecoverContainerWorker.perform_async alert.sftp_container.global_id, alert.global_id
      end
    end
    ...
    if alert.active? && (alert.container && alert.container.service.auto_scale)
      ...
      if %w[ContainerCpuUsage ContainerMemoryUsage].include? alert.name
        ...
        alert_triggered = container_service.alert_notifications.where(
          Arel.sql(%( alert_notifications.name = '#{alert.name}' AND alert_notifications.created_at > '#{5.minutes.ago.iso8601}' ))
        ).exists?
        ...
        unless alert_triggered
          ...
          ContainerServiceWorkers::AutoScaleServiceWorker.perform_async container_service.global_id, resource_kind
        end
      end
