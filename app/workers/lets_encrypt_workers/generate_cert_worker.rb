module LetsEncryptWorkers
  ...
  class GenerateCertWorker
    ...
    sidekiq_options retry: false, queue: "acme"
    ...
    def perform(lets_encrypt_id, event_id = nil)
      ...
      unless Setting.le_auto_enabled?
        event&.cancel!("Provisioning temporarily disabled")
        return
      end
      ...
      # Determine if we need a new account.
      unless lets_encrypt.has_default_acme_provider?
        lets_encrypt.update account: LetsEncryptAccount.find_or_create
      end
      ...
      order.authorizations.each do |auth|
        ...
        while %w[pending processing].include?(auth_challenge.status) && (Time.now < challenge_timeout)
          sleep(1)
          auth_challenge.reload
        end
      ...
      while %w[pending processing].include?(order.status) && (Time.now < ready_timeout)
        sleep(1)
        order.reload
      end

      # Capture invalid orders
      unless %w[valid ready].include?(order.status)
        event.event_details.create!(
      ...
      while %w[pending processing].include?(order.status) && (Time.now < order_timeout)
        sleep(1)
        order.reload
      end
      ...
      if defined?(event) && event # Should never be null!
        ...
        when /too many certificates already issued for exact set of domains/
          event.event_details.create!(
            data: "ACME RateLimited: #{e.message}",
      ...
      if lets_encrypt.account.setup!
        ...
        if defined?(event) && event
          event.event_details.create!(
            data: "ACME Account error, retrying...",
        ...
        if defined?(event) && event
          event.event_details.create!(
            data: "Fatal ACME error, contact support",
      ...
      if defined?(event) && event
        event.event_details.create!(
          data: "ACME Server Timeout. Will retry in 2 minutes.\n\n#{e.message}",
      ...
      if defined?(event) && event
        event.event_details.create!(
          data: "ACME Service Error. Will retry in 2 minutes.\n\n#{e.message}",
