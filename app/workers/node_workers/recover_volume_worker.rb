module NodeWorkers
  ...
  class RecoverVolumeWorker
    ...
    def perform(node_id)
      ...
      node.volumes.where(Arel.sql(%(created_at >= '#{(node.disconnected_at - 5.minutes).iso8601}'))).each do |vol|
        ...
        failed_event_codes = %w[7644f82e2a97cd0e cec8b7094470e525 b4e181611d7c5423]

        if event.event_details.where(event_code: failed_event_codes).exists?
