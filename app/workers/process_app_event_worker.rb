...
class ProcessAppEventWorker
  ...
  def perform(alert_name, user_id, obj_id, data = {})
    ...
    SystemNotification.where(Arel.sql(%('#{alert_name}' = ANY (rules)))).each do |i|
      if data.empty?
        next if obj.nil?
    ...
    ProjectNotification.where(Arel.sql(%(deployments.user_id = #{user.id} AND '#{alert_name}' = ANY (rules)))).joins(:user).each do |i|
      if data.empty?
        next if obj.nil?
        i.fire_app_alert! obj, alert_name
      else
    ...
    UserNotification.where(Arel.sql(%('#{alert_name}' = ANY (rules)))).where(user: user).each do |i|
      if data.empty?
        next if obj.nil?
        i.fire_app_alert! obj, alert_name
      else
