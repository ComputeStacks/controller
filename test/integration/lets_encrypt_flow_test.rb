...
#
# To re-run, be sure to run a local ACME server with:
#
#     docker run -it --rm \
#       -e GODEBUG="tls13=1" \
...
class LetsEncryptFlowTest < ActionDispatch::IntegrationTest
  ...
  test 'can enable lets encrypt on a domain' do
    VCR.use_cassette('lets_encrypt/generate_domain_cert') do
      Sidekiq::Testing.inline! do
        # Create the domain and enable Lets Encrypt
        domain = Deployment::ContainerDomain.create!(
      ...
      # TODO: Find a way to make this test work using VCR.
      #       Right now it generates a key on each test, which won't
      #       match the cert stored in VCR.
