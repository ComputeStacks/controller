...
class Deployment::ContainerDomainTest < ActiveSupport::TestCase
  ...
  test "hsts_value composes directives from the per-domain toggles" do
    d = Deployment::ContainerDomain.new
    assert_equal "max-age=63072000", d.hsts_value

    d.hsts_include_subdomains = true
    assert_equal "max-age=63072000; includeSubDomains", d.hsts_value

    d.hsts_preload = true
    assert_equal "max-age=63072000; includeSubDomains; preload", d.hsts_value

    # preload is never advertised without includeSubDomains (HSTS preload list ineligible)
    d.hsts_include_subdomains = false
    assert_equal "max-age=63072000", d.hsts_value
  end
  ...
  test "domain with valid ip" do
    requires_external_infra! # needs PowerDNS + ACME for DNS validation
